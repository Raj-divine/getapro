...
export default function BookingSection() {
  ...
  return (
    <ScrollArea className='flex-1 bg-white rounded-lg p-4 flex flex-col justify-between'>
      <div>
        <div>
        ...
        <div className='flex flex-col item-center mt-5 w-full'>
          ...
          <div className='flex justify-center'>
            <Calendar
              ...
              disabled={{
                before: new Date(),
                after: new Date(Date.now() + 1000 * 60 * 60 * 24 * 30),
              }}
            />
          </div>
          <Separator />
        ...
        <div className='mt-5 flex flex-col justify-between'>
          <div>
            <p className='text'>
              Available slots for{' '}
              {formatDate(date || new Date(), 'cccc, do MMMM, yyyy')}
            </p>
            <p className='text-sm text-gray-500'>
              You can choose multiple adjacent slots for a longer session.
              Maximum duration of a session can only be 2 hours.
            </p>
            <div className='mt-5'>
              <div className='font-heading flex flex-wrap max-sm:justify-center'>
                <div className='sm:h-40 max-sm:grid max-sm:grid-cols-2 max-sm:w-full'>
                  <MultiSelectButtons
                    options={options}
      ...
      <div className='mt-2 flex flex-col'>
        {sessions.length < 4 ? (
          ...
          <div className='flex items-start'>
            <p className='inline'>
              You have chosen {sessions.length} different sessions lasting{' '}

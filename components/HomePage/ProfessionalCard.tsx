...
export default function ProfessionalCard({
  ...
  return (
    <div className='h-full bg-white border cursor-default border-violet-200 rounded overflow-hidden'>
      ...
      <div className='p-3 flex flex-col justify-between h-[55%]'>
        ...
        <div>
          ...
          <div className='flex'>
            {specialties.map((specialty, index) => (
              <Badge
                key={index}
                className='border-primary font-medium mr-2'
                variant='outline'
              >
                {specialty}
              </Badge>
        ...
        <Button variant='primary' className='w-full h-9'>
          Book a Session
        </Button>
      </div>
    </div>
